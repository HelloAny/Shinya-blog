...
class ArticleTabbar extends React.Component {
  ...
  componentDidMount() {
    ...
    document
      ...
      .addEventListener("scroll", this._navScroll);
  }

  componentWillUnmount() {
    document
      .getElementById("anchor")
      .removeEventListener("scroll", this._navScroll);
  }
  ...
  _changeHash = (spot) => {
    ...
    this.state.objItem.forEach((k, v) => {
      document
        .getElementById(`list-${k.id}`)
        .classList.remove("acTabbar__main-text-hl");
    });
    document
      .getElementById(`list-${spot}`)
      .classList.add("acTabbar__main-text-hl");
