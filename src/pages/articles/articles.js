...
class Articles extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props
      .dispatchArticleIndex({ equal: "_id", equalVal: id })
      .then((res) => {
        domotion(false);
          ...
          <>
            <article className="article-container">
              <ArticleContent id={id} content={articleInfo.attributes.article}>
                <header className="article-container__header">
                  {articleInfo.attributes.title}
                </header>
                <time className="article-container__time">
                  {GMTToStr(articleInfo.attributes.ct)}
                </time>
              </ArticleContent>
            </article>
